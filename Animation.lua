...
-- Original code by andristum for fivem : https://github.com/andristum/dpemotes/tree/master/Client
...
-----------------------------------------------------------------------------------------------------------
   ...
   ["atm"] = {"Scenario", "PROP_HUMAN_ATM", "ATM", forceStop = true},
   ["bbq"] = {"MaleScenario", "PROP_HUMAN_BBQ", "BBQ"},
   ["bumbin"] = {"Scenario", "PROP_HUMAN_BUM_BIN", "Bum Bin", forceStop = true},
...
local function playAnimation(animData)
    ...
    if animData.AnimationOptions then
      ...
      elseif animData.AnimationOptions.EmoteMoving then
        MovementType = 51
      elseif animData.AnimationOptions.EmoteMoving == false then
        MovementType = 0
...
for k,v in spairs(DP.Emotes, function(t, a, b) return t[a][3] < t[b][3] end) do
...
for k,v in spairs(DP.PropEmotes, function(t, a, b) return t[a][3] < t[b][3] end) do
    local options <const> = {"play", "stop"}
...
for k,v in spairs(DP.Dances, function(t, a, b) return t[a][3] < t[b][3] end) do
    local options <const> = {"play", "stop"}
...
local useBackpack = false
local function loadBackpack()
    Prop = 'p_michael_backpack_s'
    PropBone = 24818
    PropPlacement = {0.07, -0.11, -0.05, 0.0, 90.0, 175.0}
    addProps(Prop, PropBone, PropPlacement[1], PropPlacement[2], PropPlacement[3], PropPlacement[4], PropPlacement[5], PropPlacement[6], PlayerProps.backpack)
end
menu.toggle(OptProp, "Backpack", {"usebackpack"}, "", function (on_change)
    if on_change then
        useBackpack = true
        loadBackpack()
    else 
        useBackpack = false
        clearProps(PlayerProps.backpack)
    end
end, false)
...
players.on_join(function(pid)
    if useBackpack then
        ...
        if pid == players.user() then loadBackpack() end
    end
end)
